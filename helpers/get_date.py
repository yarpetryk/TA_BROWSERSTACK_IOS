...
def past_date():
    date_format = "%-d. %B %Y"
    date_now = datetime.now(tz=pytz.timezone('CET'))
    current_date = date_now.strftime(date_format)
    past_date = (date_now - timedelta(days=5)).strftime(date_format)
    timestamp = (date_now - timedelta(days=5)).replace(hour=1, minute=0, second=0).strftime("%s")
    print(timestamp)
    return {
...
def get_month():
    ...
    return {
        "current_month": current_month.strftime(date_format),
