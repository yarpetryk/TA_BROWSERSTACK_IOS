...
class LoginPage(BasePage):
    ...
    def enter_username(self, text):
        self.find_element_and_send_key(LoginPageLocators.USERNAME_FIELD, f"{text}\n")
        #el = self.find_element(LoginPageLocators.USERNAME_FIELD)
        #el.clear() and el.send_keys(text)

    @allure.step("And user enter correct password")
    def enter_password(self, text):
        self.find_element_and_send_key(LoginPageLocators.PASSWORD_FIELD, text)
        #el = self.find_element_and_send_key(LoginPageLocators.PASSWORD_FIELD, text)

    @allure.step("And user click login button")
    def click_login_button(self):
        el = self.find_all_elements(LoginPageLocators.LOGIN_BUTTON)[1]
        el.click()
    ...
    @allure.step("And user hidekeyboard")
    def hide_keyboard(self):
        self.find_element_and_click(LoginPageLocators.HIDE_KEYBOARD_BUTTON)
