...
class TariffPage(BasePage):
    ...
    @allure.step("And user click on the device picker")
    def click_device_picker(self):
        self.find_element_and_click(TariffPageLocators.DEVICE_PICKER)

    @allure.step("And user select device")
    def select_device(self):
        el = self.find_all_elements(TariffPageLocators.PROSUMER_DEVICE)
        el[0].click()
    ...
    def select_day(self, month):
        prev_month = month.split()[0]
        self.find_element_and_click((By.XPATH, TariffPageLocators.SELECT_DAY.format(month=prev_month)))

    @allure.step("And user close calendar")
    def close_calendar(self):
        self.find_element_and_click(TariffPageLocators.CLOSE_CALENDAR)
