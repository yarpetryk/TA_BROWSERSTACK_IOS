...
def power_group_analytics(login, password):
    ...
    total_consumption_sum = str(round((float(consumption_sum.replace(',', '.')) + float(own_consumption_sum.replace(',', '.'))), 2))

    return {
        "consumption_sum": consumption_sum,
        "own_consumption_sum": own_consumption_sum,
